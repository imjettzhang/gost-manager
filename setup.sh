...
# 项目下载地址
ZIP_URL="https://github.com/imjettzhang/gost-manager/archive/refs/heads/main.zip"
ZIP_FILE="main.zip"
PROJECT_DIR="gost-manager-main"
...
main() {
    ...
    while true; do
        read -p "[信息] 请输入 gost 或直接回车启动管理菜单: " user_input
        if [[ -z "$user_input" || "$user_input" == "gost" ]]; then
            gost
            break
        else
            echo "[错误] 无效输入，请输入 gost 或直接回车。"
        fi
    done
}
